    ...
    for (const title of elementsToFix.querySelectorAll(".tabbed-content > .title")) {
        title.addEventListener("click", e => {
            ...
            for (const el of e.target.parentNode.children) {
                if (el.classList.contains("title")) el.classList.remove("active");
            }
...
function fixHtml(elementsToFix) {
    ...
    for (const searchContainer of elementsToFix.querySelectorAll("[data-entitysearch-route]")) {
        new Camdram.autocomplete(searchContainer);
    }
...
Camdram.slideOut = function (el, delay, duration) {
    el.style.transition = `all ${duration}ms ease-out ${delay}ms`;
    el.style.position = "relative";
    el.style.marginTop = "-4em";
    el.style.opacity = 0;
    window.setTimeout(() => el.parentNode.removeChild(el), delay + duration);
};
...
Dropzone.options.imageUpload = {
    ...
    init: function() {
        ...
        this.on("error", function(file, errorMessage, _) {
            this.removeAllFiles();
